...
<section id="details">
          <div id="details-wrapper">
            ...
            <p id="details-category">Category: <span id="categories">${item.category}</span>
            </p>
            <p id="details-salary">
            ...
            <p>Applications: <strong id="applications">${totalLikesCount}</strong></p><!--?-->
              ...
              if (userLiked == 0) {
                if (isLoggedIn && !isOwner) {
                  ...
                  <!--Like button: only for logged-in users ( not authors -->
                    ...
                    <a href="javascript:void(0)" @click=${onClickLike} id="apply-btn">Apply</a>
                  `;
                }
              }
            })()}
...
export async function detailsPage(ctx) {
  ...
  async function onClickLike() {
    const likes = {
      offerId: itemId,//changeNameId: check data.js for URL
    };
    await like(likes);
    totalLikesCount = await getTotalLikes(itemId);
    userLiked = await didUserLiked(itemId, userId);
